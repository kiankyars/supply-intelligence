...
Status: executable gross-envelope import, 2026-07-17.
...
Run the checked Abilene selection from `supply_intelligence`:

```sh
python3 -m supply_intelligence import-datacenter-power \
  --release-dir ../datacenter_atlas/releases/2026-07-17-open-seed \
...
`evidence.csv`. It copies the capacity and status evidence records, preserves site identity and scope
labels, and records the source rows and manifest hash in the lineage object. Any hash, cutoff, entity,
scope, freshness, range, target-date, or evidence mismatch fails the import.
